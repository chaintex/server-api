...
import (
	...
	// nFetcher "github.com/chaintex/server-api/fetcher/normal-fetcher"
)
...
func NewFetcher(chainTexENV string) (*Fetcher, error) {
	...
	httpFetcher := NewHTTPFetcher(infoData.APIEndpoint)
...
//mapTokenAPITokens map tokenApis to tokens
func mapTokenAPITokens(tokenApis []tomochain.TokenAPI) []tomochain.Token {
	listToken := []tomochain.Token{}
	for _, tokenAPI := range tokenApis {
		token := tomochain.Token{
			Name:    tokenAPI.Name,
...
func (fetcher *Fetcher) UpdateListToken() error {
	tokens := mapTokenAPITokens(fetcher.info.TokenAPI)
...
func (fetcher *Fetcher) CheckChainTeXEnable() (bool, error) {
	...
	return false, errors.New("Cannot check chaintex enable")
}

func getAmountInWei(amount float64) *big.Int {
	amountFloat := big.NewFloat(amount)
	tomoFloat := big.NewFloat(TOMO_TO_WEI)
	weiFloat := big.NewFloat(0).Mul(amountFloat, tomoFloat)
...
func getAmountTokenWithMinTOMO(rate *big.Int, decimal int) *big.Int {
	...
	tomoFloat := big.NewFloat(TOMO_TO_WEI)
	amoutnToken1TOMO := rFloat.Quo(rFloat, tomoFloat)
...
func (fetcher *Fetcher) makeDataGetRate(listTokens map[string]tomochain.Token, rates []tomochain.Rate) ([]string, []string, []string, []string, []*big.Int) {
	...
	for _, t := range listTokens {
		...
		destAddr = append(destAddr, tomoAddr)
		sourceSymbol = append(sourceSymbol, t.Symbol)
		destSymbol = append(destSymbol, tomoSymbol)
